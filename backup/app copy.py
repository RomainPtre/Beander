...
if st.button('Go fetch, Beander!'):
    ...
    with col2:
        ...
        df_coffee_reco = df_coffee_reco[recommend]
        ...
        df_coffee_reco = df_coffee_reco[columns_to_keep]

        # Fill a column with all values from tasting profile for fancy display of tasting profile
        df_coffee_reco['Tasting profile'] = np.nan
        df_coffee_reco['Tasting profile'] = df_coffee_reco.apply(lambda x: [x['Aroma'], x['Aftertaste'], x['Acidity'], x['Body'], x['Sweetness']], axis=1)
...
if 'df_coffee_reco' in locals():
    ...
    filtered_df = df_coffee_reco[df_coffee_reco['Variety'].isin(variety_filter)]
